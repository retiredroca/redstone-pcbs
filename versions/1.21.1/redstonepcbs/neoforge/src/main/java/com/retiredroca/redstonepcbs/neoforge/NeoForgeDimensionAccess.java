...
import com.retiredroca.redstonepcbs.neoforge.mixin.MappedRegistryAccessor;
import com.retiredroca.redstonepcbs.neoforge.mixin.MinecraftServerAccessor;
...
/** NeoForge-side implementation of the two operations that need direct access to server internals. */
public final class NeoForgeDimensionAccess implements RuntimeDimension.Access {
    public static final NeoForgeDimensionAccess INSTANCE = new NeoForgeDimensionAccess();

    private NeoForgeDimensionAccess() {}
    ...
    public ServerLevel createAndRegisterLevel(MinecraftServer server, ResourceKey<Level> key, LevelStem stem) {
        ...
        ServerLevel level = new ServerLevel(
                ...
                false,
                null);
        ((DimensionLevelRegistrar) server).redstonepcbs$registerLevel(level);
        return level;
    }
}
